...
export default ['$scope', '$rootScope', '$stateParams', 'GearForm', 'Rest',
    'ProcessErrors', 'GetBasePath', 'Wait', 'CreateSelect2',
    '$state', 'i18n','ParseTypeChange','ParseVariableString', 'ToJSON',
    function($scope, $rootScope, $stateParams, GearForm, Rest, ProcessErrors,
GetBasePath, Wait, CreateSelect2, $state, i18n, ParseTypeChange, ParseVariableString, ToJSON) {
        ...
        function init() {
            ...
            Rest.get(defaultUrl).then(({data}) => {
                    ...
                    $scope.networkgear_id = id;
                    $scope.networkgear_obj = data;
                ...
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, null, {
                        hdr: i18n._('Error!'),
                        msg: i18n.sprintf(i18n._('Failed to retrieve network gear: %s. GET status: '), $stateParams.id) + status
                    });
                });
        ...
        $scope.formCancel = function() {
            $state.go('resourceNetworkGearsList', null, { reload: true });
        };
