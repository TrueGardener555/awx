...
export default ['$scope', '$rootScope', 'RegistryForm', 'GenerateForm', 'Rest',
	...
	$state, $location, i18n, ParseTypeChange, ToJSON) {
        ...
        function init() {
            ...
            CreateSelect2({
                element: '#registry_datacenter',
                multiple: false,
            });
            CreateSelect2({
                element: '#registry_credential',
                multiple: false,
            });
        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_registries') {
                        $rootScope.flashMessage = i18n._('New registry successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "registry", data.id);
                        $state.go('resourceRegistriesList.edit', { registry_id: data.id }, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new registry. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('resourceRegistriesList');
        };
    }
];
