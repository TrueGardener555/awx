...
GetBasePath, Wait, CreateSelect2, $state, i18n, ParseTypeChange, ParseVariableString, ToJSON) {
        ...
        function init() {
            ...
            Rest.get(defaultUrl).then(({data}) => {
                    ...
                    $scope.registry_id = id;
                    $scope.registry_obj = data;
                ...
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, null, {
                        hdr: i18n._('Error!'),
                        msg: i18n.sprintf(i18n._('Failed to retrieve registry: %s. GET status: '), $stateParams.id) + status
                    });
                });
        ...
        $scope.formCancel = function() {
            $state.go('resourceRegistriesList', null, { reload: true });
        };
