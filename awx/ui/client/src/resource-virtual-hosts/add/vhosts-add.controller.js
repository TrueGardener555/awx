        ...
        $scope.formSave = function() {
            ...
            Rest.post(data)
                .then(({data}) => {
                    ...
                    if (base === 'ipam_virtual_hosts') {
                        $rootScope.flashMessage = i18n._('New virtual host successfully created!');
                        $rootScope.$broadcast("EditIndicatorChange", "virtual host", data.id);
                        $state.go('resourceVirtualHostsList.edit', { vhost_id: data.id }, { reload: true });
                    } else {
                        ReturnToCaller(1);
                    }
                })
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, form, { hdr: i18n._('Error!'), msg: i18n._('Failed to add new virtual host. POST returned status: ') + status });
                });
        };

        $scope.formCancel = function() {
            $state.go('resourceVirtualHostsList');
        };
    }
];
