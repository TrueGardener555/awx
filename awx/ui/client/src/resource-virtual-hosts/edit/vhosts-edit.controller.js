...
export default ['$scope', '$rootScope', '$stateParams', 'VHostForm', 'Rest',
    'ProcessErrors', 'GetBasePath', 'Wait', 'CreateSelect2',
    '$state', 'i18n','ParseTypeChange','ParseVariableString', 'ToJSON',
    function($scope, $rootScope, $stateParams, VHostForm, Rest, ProcessErrors,
GetBasePath, Wait, CreateSelect2, $state, i18n, ParseTypeChange, ParseVariableString, ToJSON) {

        var form = VHostForm,
            master = {},
            id = $stateParams.vhost_id,
            defaultUrl = GetBasePath('ipam_virtual_hosts') + id;
            ...
            Rest.get(defaultUrl).then(({data}) => {
                ...
                .catch(({data, status}) => {
                    ProcessErrors($scope, data, status, null, {
                        hdr: i18n._('Error!'),
                        msg: i18n.sprintf(i18n._('Failed to retrieve virtual host: %s. GET status: '), $stateParams.id) + status
                    });
                });
        ...
        $scope.formCancel = function() {
            $state.go('resourceVirtualHostsList', null, { reload: true });
        };
